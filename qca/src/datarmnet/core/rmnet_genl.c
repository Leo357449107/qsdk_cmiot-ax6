				     ...
				     struct genl_info *info)
{
	...
	pid_bps_req = kzalloc(sizeof(*pid_bps_req), GFP_ATOMIC);
	if(!pid_bps_req)
		return -ENOMEM;

	pid_bps_resp = kzalloc(sizeof(*pid_bps_resp), GFP_ATOMIC);
	if(!pid_bps_resp){
		kfree(pid_bps_req);
		return -ENOMEM;
	}
				       ...
				       struct genl_info *info)
{
	...
	rm_err("%s", "CORE_GNL: rmnet_core_genl_pid_boost_req_hdlr");
